#!/usr/bin/env bash
# Shared tail of the updaters: validate a version, write it into ./PKGBUILD, print it.
set -euo pipefail
...
if [ "$ver" != "$cur" ]; then
  # Never go backwards: stale upstream metadata would look like a pacman downgrade.
